...
/* 90Y */
/* UPPER LIMIT, D0_UPPER = 3100 eV, FG05 SPIN CUTOFF E&B 2005/2006 */
...
/* Declare parameters */
const double T_ct  = 1.000;      // temperature parameter
const double E0    =-1.431;  // shift for the CT level density
 
const double a_par = 9.050;     // level density parameter
const double A     = 90;      // mass A
const double E1    = -1.231;     // Fermi-gas shift for the spin cutoff parameter, E&B 2005/2006
const double Pa = -1.700; //  deuteron pairing shift Pa' for the spin cutoff parameter, E&B 2009
...
int main(){
	...
	ifstream inputEx89Yfile("ex_and_T_90Y.txt");
	FILE *output89Yfile;
    output89Yfile = fopen("ct_90Y_ldmodel4_format.txt","w");
    
	/* Vectors for calculated level densities. */
    double e_calc[56], temp_calc[56], nld_calc[56];
    double start_spin = 0.;    // the first spin, 1/2 (odd) or 0 (even). Loop up to 59/2 = 29.5 if odd spin, up to 29 (starting from 0) if even spin
