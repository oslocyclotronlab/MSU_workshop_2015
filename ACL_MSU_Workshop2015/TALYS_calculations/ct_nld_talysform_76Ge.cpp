...
int main(){
	...
	ifstream inputEx76Gefile("ex_and_T_76Ge.txt");
	FILE *output76Gefile;
    output76Gefile = fopen("ct_76Ge_ldmodel4_format.txt","w");
    
	/* Vectors for calculated level densities. */
    double e_calc[56], temp_calc[56], nld_calc[56];
    double start_spin = 0.0;    // the first spin, 1/2 (odd) or 0 (even). Loop up to 59/2 = 29.5 if odd spin, up to 29 (starting from 0) if even spin
    ...
    getline(inputEx76Gefile,line);
    getline(inputEx76Gefile,line);
    i=0;
    while(inputEx76Gefile){
        inputEx76Gefile >> x >> y;
        e_calc[i]    = x;
        temp_calc[i] = y;
	...
	for(i=0; i<55; i++){
        ...
        for(int j=0;j<stop_spin;j++){
            ...
            fprintf(output76Gefile,"%8.2E ",x);
        }
        fprintf(output76Gefile,"\n");
        
	}
	
	
	/* Close files */	
	inputEx76Gefile.close();
	fclose(output76Gefile);
